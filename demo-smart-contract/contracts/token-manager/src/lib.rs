...
pub mod token_manager {
    ...
    /// Transfer 500 DEMO-QTC to specific wallet
    pub fn transfer_to_phantom_wallet(
        ctx: Context<TransferToPhantom>
    ) -> Result<()> {
        let amount = 500_000_000; // 500 tokens * 10^6 decimals
        ...
        msg!("Transferred 500 DEMO-QTC to Phantom wallet: {}",
...
pub struct TransferToPhantom<'info> {
    #[account(mut)]
    pub from_token_account: Account<'info, TokenAccount>, // Parent wallet token account
    #[account(mut)]
    pub to_token_account: Account<'info, TokenAccount>,   // Phantom wallet token account
